...
export function CvModal({ onClose }: CvModalProps) {
  ...
  return <Modal onClose={onClose} labelledBy="cv-title" className="cv-modal">
    ...
    <div className={`cv-preview-wrap ${tab === "preview" ? "" : "print-only"}`}>
      <article className="cv-paper" id="cv-print">{!hasOwnData && <div className="cv-demo-notice">Eksempel-CV – dine egne opplysninger vises her når de er lagt inn</div>}<header><h1>CV – {preview.fullName || "Ditt navn"}</h1><div className="cv-contact">{preview.address && <span>Adresse: {preview.address}</span>}{preview.email && <span>E-post: {preview.email}</span>}{preview.phone && <span>Telefon: {preview.phone}</span>}{preview.birthDate && <span>Født: {preview.birthDate}</span>}{preview.familyStatus && <span>Sivilstatus: {preview.familyStatus}</span>}{preview.linkedin && <span>LinkedIn Profil: {preview.linkedin}</span>}</div></header>
